...
for dataset in combine:
    ...
    for i in range(0, 2):
        for j in range(0, 5):
            guess_df = dataset[(dataset['Sex'] == i) & (['Title'] == j + 1)]['Age'].dropna()
            # print(guess_df)
            age_guess = guess_df.median()
            # print("age_guess: ", age_guess, " sex = ", i, "title: ", j + 1)
            if (not np.isnan(age_guess)):
                guess_age[i, j] = int(age_guess / 0.5 + 0.5) * 0.5

    for i in range(0, 2):
        for j in range(0, 5):
            dataset.loc[(dataset.Age.isnull()) & (dataset.Sex == i) & (dataset.Title == j + 1), 'Age'] = guess_age[
                i, j]
